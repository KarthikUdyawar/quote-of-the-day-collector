...
class AsyncServiceBase(Generic[T]):
    def __init__(self, model: Type[T]):
        """
        Initialize the service with the ORM model type.

        Parameters:
            model (Type[T]): ORM model class that this service will operate on.
        """
        self.model = model
    ...
    async def delete(self, obj: T) -> None:
        ...
        Parameters:
            obj (T): An instance of the service's model to delete from the database.
        """
        async with get_db() as session:
